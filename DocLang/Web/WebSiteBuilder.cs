...
namespace BassClefStudio.DocLang.Sites
{
    ...
    public class WebSiteBuilder : ISiteBuilder
    {
        ...
        /// <summary>
        /// The default key for <see cref="IDocFormatter"/> and <see cref="IDocValidator"/> instances used to validate and convert document files.
        /// </summary>
        ...
        public async Task<IStorageFolder> BuildSiteAsync(IStorageFolder source)
        {
            ...
            IStorageFolder output = await source.CreateFolderAsync("site", CollisionOptions.Overwrite);
            ...
            using (var fileStream = await configFile.OpenFileAsync(FileOpenMode.Read))
            {
                ...
                foreach (var template in config.Root.Elements("Template"))
                {
                ...
                foreach (var style in config.Root.Elements("Style"))
                {
                ...
                foreach (var asset in config.Root.Elements("Asset"))
                {
                ...
                XmlWriterSettings writeSettings = new XmlWriterSettings() { Async = true, OmitXmlDeclaration = true };
                XmlReaderSettings readSettings = new XmlReaderSettings() { Async = true, DtdProcessing = DtdProcessing.Parse };

                foreach (var page in config.Root.Elements("Page"))
                {
                    var path = page.Value;
                    if (string.IsNullOrEmpty(path))
                    {
                    ...
                    else
                    {
                        ...
                        string key = page.Attribute("Template")?.Value ?? DefaultKey;
